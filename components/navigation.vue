	...
	>
		<div class="navigation-top layout">
			<div class="navigation-top__item phones">
				...
				<a
					...
					:href="`tel:${$store.state.contacts.number_1_call}`"
				>{{ $store.state.contacts.number_1 }}</a>
				<a
					class="binct-phone-number-2"
					:href="`tel:${$store.state.contacts.number_2_call}`"
				>{{ $store.state.contacts.number_2 }}</a>
			</div>
			<div class="navigation-top__item">
				...
				<i
					...
					aria-hidden="true"
				></i>
				<a :href="`mailto:${$store.state.contacts.email}`">{{ $store.state.contacts.email }}</a>
			</div>
			<div class="navigation-top__item lang">
				...
				<nuxt-link
					...
					:to="switchLocalePath($i18n.locale === 'uk' ? 'ru' : 'uk')"
				>{{ $i18n.locale }}</nuxt-link>
			</div>
		</div>
		...
		<nav class="navigation layout">
			...
			>
				...
				>
					<nuxt-link
						:to="localePath(`/services/${c.id}`)"
						class="service-category__title"
					>{{ $t(`services.${c.id}`) }}</nuxt-link>
					<div v-if="c.jobs">
						<div
							:key="j"
							v-for="j in c.jobs"
			...
			<div
				class="navigation-content-right"
			...
			>
				...
				<nuxt-link
					:to="localePath('/about-business-point')"
					class="navigation-link"
				>
					{{ $t('navigation.about')}}
				</nuxt-link>
				...
				<a
					target="_blank"
					href="/catalog.pdf"
					class="navigation-link download"
				>
